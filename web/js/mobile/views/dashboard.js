...
import { agentRouteQuery } from '../../shared/agent-helpers.js';
    ...
    if (canPatch) {
        ...
        <div class="agent-list">
          ${filtered.length === 0
            ...
            : filtered.map(agent => {
                ...
                return `
                <div class="agent-card${isEditing ? ' editing' : ''}" data-id="${agent.id}" data-host-key="${escapeHtml(hostKeyFromAgent(agent))}">
                  ...
                  <div class="agent-card-meta">
                    ...
                    ${agent.context_name ? `<span class="agent-ctx">${escapeHtml(agent.context_name)}</span>` : ''}
      ...
      listEl.querySelectorAll('.agent-card').forEach(el => {
        el.addEventListener('click', () => {
          ...
          const hostKey = el.dataset.hostKey || '';
          const agent = agents.find(a => a.id === id && hostKeyFromAgent(a) === hostKey)
            || agents.find(a => a.id === id);
          navigate(`/agent/${id}${agent ? agentRouteQuery(agent) : ''}`);
        });
      });

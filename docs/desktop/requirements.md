...
| CAM-DESK-CRON-010 | approved | Agent Settings > Cron maps directly to node-local `camc cron` on the selected agent's resolved context. The Hub exposes `GET /api/agents/{id}/cron` -> `camc cron list --json`, `POST /api/agents/{id}/cron` -> `camc cron add`, and `DELETE /api/agents/{id}/cron/{job}` -> `camc cron rm`. CamUI-created jobs use the managed name prefix `agent-<agentId>-<uiName>` so the agent Cron tab can show only jobs attached to the selected agent while leaving other node-local cron jobs visible through normal `camc cron list`. P0 card UI provides: target summary, Add cron job card (name, schedule type every/daily/in/at, schedule value, message, timeout, max attempts, TTL/no-expire), Refresh, and per-job cards with schedule/host/attempts/last-status/remove. `POST` builds a shell command `~/.cam/camc msg send <agentId> -t <message> --no-wait` and stores it through `camc cron add --shell ...` with `--cwd` set to the agent workspace. The Hub continues to use `_contextForAgent` + `_execCamcOnContext`, so pooled SSH, bundled `~/.cam/camc` bootstrap, credentials, and Direct mode security rules are reused. Manual JSON editing under `~/.cam/cron/jobs.d` remains out of scope; the UI is a controlled wrapper over `camc cron` commands. |
...
| CAM-DESK-EDIT-016 | approved | The agent sidebar exposes a per-row `⋯` action menu with exactly `Settings`, `Stop`, and `Remove`; Stop calls `CamApi.stopAgent(id)` and Remove calls `CamApi.deleteAgentHistory(id)`, which Direct mode maps through CAM-DESK-DIRECT-020 to `camc stop` and `camc rm --kill`. `Settings` opens a full Agent Settings surface inside Agents mode, styled like the main Settings page with tabs for `Attributes`, `Loop`, `Workflow`, and `Cron`. P0 makes `Attributes` editable: agent name, auto-confirm toggle, and comma-separated tags. Save calls `CamApi.updateAgent(id, body)` (PATCH `/api/agents/{id}`) with `{ name?: string, auto_confirm?: boolean, tags_add?: string[], tags_remove?: string[] }`; the embedded Hub translates to `camc update <id> [--name X] [--auto-confirm true|false] [--tag T]… [--untag T]…`. Tags must match `^[A-Za-z0-9_-]{1,32}$`; invalid tags surface inline and skip the request. `Loop` and `Workflow` remain visible planning tabs for the next automation slices; `Cron` is implemented as the card-style node-local scheduler surface described by CAM-DESK-CRON-010. The settings surface never exposes credentials, system prompts, or AGENTS.md content until upstream camc has a safe update API. |

...
def cmd_msg_send(args):
    ...
    # Resolve target name / short-id / full-id → tmux session (same path as `camc send`)
    store = AgentStore()
    ...
    while time.time() < deadline:
        ...
        while body:
            t = body[-1].strip()
            if (not t or t in _MSG_PROMPT_CHARS
        ...
        # Stable check: same content for `stable_for` consecutive captures

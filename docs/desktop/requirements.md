...
| **Relay** | "Use an existing CAM Hub through a relay endpoint." | Relay URL + relay token. (CAM API token is profile-managed on the source side — not user-facing in normal flow.) Relay proxies the same REST/WS surface. |
...
| CAM-DESK-SET-002 | approved | Settings includes an **Appearance** tab for desktop-only display preferences. It must provide theme selection (`dark`, `light`, `system`) with dark as the default, plus conservative UI and agent-output font-size controls. Settings persist in localStorage, apply immediately without reload through `body.desktop` data attributes/CSS custom properties, and must not edit mobile/PWA files. The agent-output font-size setting applies to Plain, Rich, Browse previews, and Terminal/xterm mode; Terminal refits after the variable changes. Light mode is a v1 variable override for readable surfaces, borders, inputs, buttons, sidebars, Settings/Start/Nodes, Plain/Rich agent output, floating output controls, rich-code blocks, and xterm terminal surfaces. | `web/desktop.html`, `web/js/desktop/settings-mode.js`, `web/css/desktop.css`, `web/js/desktop/agent-console.js`. |
...
| CAM-DESK-DIRECT-011 | approved | Electron main owns Direct Hub lifecycle: start/check/stop the embedded Electron/Node hub, generate an API token, bind only to loopback, and keep lifecycle state inside the app. Direct first tries the preferred fixed loopback range `8420..8469`; if that range cannot be bound (for example Windows excluded port ranges return `EACCES`, or another platform blocks the preferred range), it must fall back to `listen(0, '127.0.0.1')`, persist/return the actual OS-assigned `apiUrl`, and keep Settings Restart usable as recovery. Direct must not spawn local `cam serve`, require local Python, require WSL, or require a host shell. The renderer cannot pass command strings, argv, env, or filesystem paths. |
...
| CAM-DESK-DIRECT-020 | approved | The agent row `⋯` menu's **Stop** and **Remove** actions are wired in Direct mode through the embedded Hub. Stop maps to `DELETE /api/agents/{id}` and runs `camc stop <id>` on the agent's resolved context for graceful termination; an agent already in a terminal status (`completed`, `failed`, `timeout`, `killed`, `stopped`) returns `{ok:true, no_op:true}` so the menu does not surface a false error. Remove maps to `DELETE /api/agents/{id}/history` and runs `camc rm <id> --kill` on the agent's resolved context; modern camc always kills tmux on `rm`, so Remove works even while the agent is running. Both paths resolve through `_contextForAgent(id)` and execute via `_execCamcOnContext(ctx, args)` so they reuse the pooled `ssh2.Client` (CAM-DESK-DIRECT-019) and the bundled `~/.cam/camc` bootstrap. On success the Hub mirrors the change into `state.store.agents` (Stop marks status `stopped` with `completed_at`; `DELETE /api/agents/{id}?force=true` maps to `camc kill <id>` and marks status `killed`; Remove drops the row) and calls `saveStore()` so the renderer's next poll cycle is in sync without waiting on `camc --json list`. Errors return clear JSON shapes with `error` ∈ `{agent_not_found (404), context_not_found (404), store_unavailable (503), stop_failed (502), remove_failed (502)}` and `detail`. No separate kill route is introduced in this pass; the existing `force=true` query parameter on `CamApi.stopAgent(id, true)` is the force-kill surface. |
| CAM-DESK-CRON-010 | approved | Agent Settings uses one `Automation` tab instead of separate Loop/Cron tabs. The add card requires an automation type: `Agent loop` sends prompt text to the selected agent when idle via `camc cron add --loop --owner <agent> --prompt <text>`; `Host cron` runs a shell command on the selected agent's resolved node via `camc cron add --shell <command>`. Host cron shows `Working directory`, defaulting to the agent workspace (`agent.context_path || agent.path || context.path`) and allowing user override; Loop hides working directory because the owner agent already runs in its session/workspace. The Hub exposes `GET /api/agents/{id}/cron` to return both CamUI-managed host cron jobs and any agent loops, `POST /api/agents/{id}/cron` with `{type:'loop'|'cron', ...}` to create the selected kind, and `DELETE /api/agents/{id}/cron/{kind}:{job}` to remove either `loop:` or `cron:` items. CamUI-created host cron jobs keep the managed name prefix `agent-<agentId>-<uiName>` so the Automation tab shows only jobs attached to the selected agent while leaving other node-local cron jobs visible through normal `camc cron list`. The Hub continues to use `_contextForAgent` + `_execCamcOnContext`, so pooled SSH, bundled `~/.cam/camc` bootstrap, credentials, and Direct mode security rules are reused. Manual JSON editing under `~/.cam/cron/jobs.d` or `~/.cam/loops` remains out of scope; the UI is a controlled wrapper over `camc cron` commands. |
...
### Skillm Library Management (SKILLM)

| ID | Status | Requirement |
| --- | --- | --- |
| CAM-DESK-SKILLM-010 | approved | Desktop exposes a **Skills** workspace mode that wraps a managed node-local `skillm` CLI. The mode has exactly two user-facing tabs: **Repositories** and **Install**. It must let the user pick a setup SSH context/node, check/deploy Skillm availability, add/remove/rename/edit/refresh named Skillm repositories on that node, list skills across all repositories or one repository, and install selected skills either globally for selected agent CLIs or into a selected workspace path. There is no standalone Sync tab/button; repository refresh is an embedded-Hub backend step before list/install and behind explicit per-repo Refresh. |
| CAM-DESK-SKILLM-011 | approved | The embedded Hub exposes `/api/skillm/status`, `/api/skillm/repos`, `/api/skillm/repos/refresh`, `/api/skillm/list`, `/api/skillm/repo-connect`, `/api/skillm/sync`, and `/api/skillm/install`. `/repo-connect` and `/sync` remain compatibility wrappers; new renderer code uses `/repos*`. Routes accept context names and execute the bundled Skillm artifact through the existing SSH transport pool. Before each command, the Hub verifies `~/.cam/skillm` by comparing its 12-char MD5 prefix with the packaged `resources/skillm/skillm` (source fallback `dist/skillm`) and uploads the bundled copy when missing, not executable, or hash-different, matching CAMC deploy/upgrade behavior. Renderer code never executes shell commands and never receives SSH credentials. Local/non-SSH contexts return a clear `not_ssh` error in v0. |
| CAM-DESK-SKILLM-012 | approved | Git tokens are one-shot form values. Desktop must not persist the token in `localStorage`, the Hub must not write it to `embedded-hub.json`, and all route responses must redact the raw token, URL-encoded token, and `oauth2:<secret>@` credential fragments from stdout/stderr/detail text. |
| CAM-DESK-SKILLM-013 | approved | Skill listing is JSON-first when `skillm` exposes a stable JSON output. Until then, the Hub may parse the current Rich table output as a compatibility fallback and must return the raw redacted stdout for diagnostics. |
| CAM-DESK-SKILLM-014 | approved | v0 uses Git pull on each node as the consistency mechanism. Repository refresh is backend-driven: opening Install, changing the repository filter, explicit per-repo Refresh, and Install Selected trigger Hub-side `skillm pull` before list/install as appropriate. It does not introduce a shared service gateway, SSH tunnel, default-skill daemon, or Desktop-owned skill database. Those remain future work documented in `docs/desktop/skillm-integration.md`. |
| CAM-DESK-SKILLM-015 | approved | Skills mode has exactly **two tabs**: **Repositories** and **Install**. The previous standalone Sync tab is gone — every list/install/repo-write path runs `skillm pull` server-side first, so the user never has to remember a separate sync step. The Repositories tab shows a per-repo row (name, redacted URL, "active"/"local-only" badge) with per-row **Edit**, **Refresh**, and **Remove** actions, plus the setup-header **Refresh** action and **Add Repository** form (name + URL + one-shot token). Edit covers rename and URL/token changes (implemented Hub-side as `skillm repo rm` + `skillm repo add` + `skillm pull` because upstream `skillm` has no first-class `repo rename` or `repo set-url`; the token-redacted URL is repopulated from `skillm repo list` so the user only re-enters credentials when actually changing them). Renaming a local-only repo (no upstream URL) returns a clear error. |
| CAM-DESK-SKILLM-016 | approved | The Install tab carries: a repository filter dropdown (All repositories / per-repo), a search box, a selectable skill list (with repo prefix shown on each row), a node-scope segmented control (Setup node / All SSH / Selected), an Install-target panel (Workspace path or Global default), an agent-CLI checkbox grid (Claude / Codex / OpenClaw / Cursor), and an **Install selected skills** button. On open the Hub does a `skillm pull` over every repo on the setup node before listing; the same `pull` runs on each install target before the install action so the user gets the freshest skills. The Hub surface for this is `GET /api/skillm/repos`, `POST /api/skillm/repos` (add), `PATCH /api/skillm/repos` (edit/rename), `DELETE /api/skillm/repos` (remove), `POST /api/skillm/repos/refresh` (one repo or all), extended `GET /api/skillm/list?context=…&repo=…&sync=1`, and the existing `POST /api/skillm/install`. All routes redact Git tokens before returning stdout/stderr, never persist them to `embedded-hub.json`, and never echo them back to the renderer in response bodies. |
...
V0 surface: a structured Markdown worklog browser, mounted as a
top-level workspace mode alongside Agents / Skills / Nodes / Settings.
The renderer-side store is local and mock-safe in V0; a future slice
wires the embedded Hub to a `todocli`-style backend.

| ID | Status | Requirement |
| --- | --- | --- |
| CAM-DESK-TODOS-010 | approved | Desktop exposes a top-level **Todos** workspace mode in the same nav as Agents / Skills / Nodes / Settings. The mode occupies the full-width content rail and uses the existing single-column card layout (no two-column sprawl) so mobile and narrow-window users see one structural list at a time. The mode is independent of the agent list: switching to Todos does not unsubscribe agent polling, and switching back to Agents restores the prior selected agent. |
| CAM-DESK-TODOS-011 | approved | The mode has four top-level tabs: **Inbox**, **Tasks**, **Projects**, **Archive**. Tasks are the only primary row type in V1. Inbox shows untriaged tasks whose project is missing or `inbox`; Tasks shows all non-archived tasks; Projects shows non-archived tasks assigned to named projects; Archive shows archived tasks. Task-scoped `notes[]` are edited inside the task detail pane, not represented as standalone note rows. |
| CAM-DESK-TODOS-012 | approved | The header carries a **store/context selector** (which worklog store the rows come from), a connection/sync **status** badge, a **search** box (case-insensitive substring on title + body), a **filter** popover (status / tag / project), and a **sort** control (Updated / Created / Title). V0 ships the controls live with local mock data; the filter/sort UI must not depend on a backend round-trip. |
| CAM-DESK-TODOS-013 | approved | The structural rail is a flat list of row cards, one per worklog item. Each card shows: kind glyph (task / note / project), title, one-line preview, optional checklist progress (e.g. `3/5`), tag chips, updated-relative-time. Clicking a row opens the detail panel. The list scrolls independently of the header controls. |
| CAM-DESK-TODOS-014 | approved | The primary creation action is **New Task**. It creates a task row and opens the inline edit form in the detail pane. Add Note is not a top-level creation action; task notes are appended from the selected task detail pane and map to todocli `note -t <task_id>`. |
| CAM-DESK-TODOS-015 | approved | The detail panel for a selected task exposes sub-tabs **Details**, **Body**, **Notes**, **Checklist**, **History** plus an inline **Edit** action. Edit covers structured metadata (title, goal, status, priority, project, tags) and the optional Markdown body in one form. Details shows the metadata goal and body preview; Body shows the raw optional Markdown/traces; Notes supports add/edit/delete for task-scoped activity notes; Checklist supports add/edit/delete plus done/undone toggles per entry; History renders safely even when fields are missing. Global AppState refreshes must not rebuild an active task/note/checklist editor or focused Todos input, so unsaved typed drafts are not flushed by background polling. |
| CAM-DESK-TODOS-016 | approved | The V0 data model is a task record shape `{ id, type:"task", title, goal, body, status, priority, tags, project, notes, checklist, history, updatedAt }` persisted to `localStorage` under a stable key so users can dogfood the UI without a backend. The model mirrors todocli after the `goal` metadata update: `goal` is structured metadata, `body` is optional Markdown/traces and may be empty, `notes[]` is a task-scoped activity/comment list, and `checklist[]` is an editable per-task list of `{id,text,done}` entries. A future slice replaces the localStorage store with a Hub-mediated `todocli` API; the renderer must not encode that backend's URL shape in the V0 store interface. |
| CAM-DESK-TODOS-017 | approved | No mobile / PWA files are touched. The Electron desktop surface lives in `web/desktop.html`, `web/js/desktop/todos-mode.js`, `web/js/desktop/app.js`, and `web/css/desktop.css` under a scoped `.todos-*` prefix. The legacy React/Tauri prototype may mirror the same concept in `apps/cam-desktop/src/components/TodosMode.tsx` plus `lib/useTodos.ts`, but the packaged Windows Electron app uses the `web/js/desktop` implementation. Switching to Todos must preserve agent polling/selection state so returning to Agents restores the prior selected agent. |
| CAM-DESK-TODOS-018 | approved | The local V0 renderer migrates older standalone `note` rows into task rows on read so no local text is discarded. Top-level Notes UI is removed; task notes live under the selected task detail. Archive is an explicit task action that sets `status:"archived"`; archived tasks leave Inbox/Tasks/Projects and appear under Archive, where **Unarchive** restores `active`. |
...
| CAM-DESK-REMOTE-012 | approved | **Relay** mode wraps the same hub/controller surface behind a relay endpoint. **As of 2026-06-12, the user-facing Relay setup requires exactly two fields: Relay URL + Relay token.** The CAM API token (the bearer the embedded Hub expects on `/api/*`) is internal to the source ↔ Hub boundary and is profile-managed on the source side, not pasted into Desktop/mobile after every source restart. Relay token authenticates the app/source connection to the relay server; CAM API token authenticates the relay/source connection to the embedded Hub. The relay (`relay/relay.py`) injects the configured `Authorization: Bearer <api-token>` into any proxied `/api/*` frame that lacks an `authorization` header, so old clients that still send the CAM API Bearer continue to work (header passthrough) and new clients can omit it entirely. The source `camui start` CLI accepts `--api-token TOKEN` / `CAMUI_API_TOKEN=TOKEN` / `--profile NAME` (reads `~/.cam/camui/relay/<NAME>/profile.json`, generates and persists `cam_api_token` once on first use). Current profile file shape is `{ "cam_api_token": "..." }`, written with mode `0600` inside a `0700` profile directory; full tokens are never logged (only the `sha256:<24hex>` fingerprint). `--show-token` remains for debugging. `/api/ws` relay streaming is still deferred; clients continue REST polling. |
| CAM-DESK-REMOTE-012a | approved | Desktop and mobile-v2 Relay settings forms must collect exactly Relay URL + Relay token. Any prior `cam_token` Relay field is removed from the active UI. Error copy on a failed Relay connect must reference Relay URL/token or source status — not the CAM API token. `web/js/api.js`'s relay HTTP/WS path must not require `cfg.token` when the connection is Relay; Direct continues to send `Authorization: Bearer <cam-api-token>` against the embedded Hub on loopback. |
...
| CAM-DESK-NODEUI-018 | approved | Expanded SSH host cards expose **Diagnose Host** next to Sync Host. The renderer calls `POST /api/contexts/{name}/diagnose` for the host's representative context, then renders an inline diagnostic card with ordered steps for context resolution, credential/key-file readiness, SSH echo, bundled `~/.cam/camc` bootstrap, `camc version`, and `camc --json list`. The diagnostic response must include timings when ssh2 provides them and must never include raw passwords, passphrases, private-key contents, or auth tokens; only non-secret metadata such as host/user/port/auth method, key-file path, credential presence, and high-level error details may be shown. Diagnose is read-only and must not modify contexts, agents, or remote files except for the existing bundled-camc bootstrap check shared with Sync Host. |
...
| CAM-DESK-RUN-011 | verified | The Start Agent form must cover the mobile/WebUI field set: tool, context, prompt, auto-confirm, auto-exit, optional name, optional timeout, and retry count. It also exposes an optional `System prompt` textarea; when non-empty, Desktop includes `system_prompt` in the `CamApi.startAgent()` body so Direct/Relay backends can map it to `camc run --system-prompt`. |
...
### Workspace Browser / File Parity

The Browse feature is a **workspace browser**, not a Context-only feature.
Context Browse in the mobile/PWA app is the current implementation reference,
but the Desktop Agent page should also expose Browse from the selected agent.
Both entry points resolve to a workspace root and then use the same list/read
contract. Relay and Direct must behave the same at the renderer boundary.

| ID | Status | Requirement |
| --- | --- | --- |
| CAM-DESK-FILE-010 | approved | Desktop must expose **Browse** as a first-class Agent Detail mode alongside Plain/Rich/Terminal. The selected agent is the entry point; Browse resolves that agent to a workspace root and opens the browser there. If the agent has a matching context, that context may supply the root and transport metadata. If no context record exists, the hub must still use the agent's own workspace/path metadata when available. |
| CAM-DESK-FILE-011 | approved | The public UI API for Browse is a workspace file contract shared by Relay and Direct. Agent-scoped endpoints are preferred for Agent Detail: `GET /api/agents/{id}/workspace/files?path=<subpath>` and `GET /api/agents/{id}/workspace/files/read?path=<subpath>`. Context-scoped endpoints remain valid compatibility entry points: `GET /api/contexts/{name_or_id}/files?path=<subpath>` and `/files/read`. Both scopes return the same response shape so the renderer can use one browser component. |
| CAM-DESK-FILE-012 | approved | List responses must match the mobile/Relay file-browser shape, with workspace metadata added where useful: `{scope, agent_id?, context_id?, context_name?, root, path, parent, entries:[{name,type,size,mtime?}]}`. `type` is at least `dir` or `file`; `symlink` may be returned when the backend can distinguish it. Entries are sorted by the renderer with directories first, then locale name order, matching mobile behavior. |
| CAM-DESK-FILE-013 | approved | Read responses must match the mobile/Relay shape: `{scope, agent_id?, context_name?, root, path, binary, size, content}`. Text content is UTF-8 with latin-1 fallback when needed. Binary content may be base64 in the API response, but the first UI pass may render only a binary placeholder with size. The first Desktop pass is read-only: no write, rename, delete, chmod, or upload-from-browser inside Browse. |
| CAM-DESK-FILE-014 | approved | Relay and Direct must implement the same Browse semantics. Relay uses the existing FastAPI/TransportFactory path. Direct embedded Hub must implement the same endpoints instead of returning silent empty stubs; for SSH nodes it should use the existing long-lived `ssh2` pool and SFTP/list/read operations. The renderer must not branch on Relay vs Direct except to display capability/error text returned by the API. |
| CAM-DESK-FILE-015 | approved | Workspace path resolution is constrained to the workspace root. For context scope, the root is `context.path`. For agent scope, the root is resolved from the agent's workspace/path metadata first, then from a matching context as fallback. Backends must reject `..` traversal and absolute-path escape; local backends must also defend against symlink escape as the current Relay route does. SSH/Direct must normalize logical paths under root and reject outside-root paths before issuing SFTP operations. |
| CAM-DESK-FILE-016 | approved | Desktop Browse UI should use a two-pane layout inside the Agent Detail Browse mode: left pane for breadcrumb + directory/file navigation, right pane for preview. This mirrors the mobile list/viewer model while fitting desktop space. Plain text/code previews must be selectable/copyable. Markdown/HTML rendering is optional in the first pass; if implemented, it must not depend on a CDN in packaged Desktop. |
| CAM-DESK-FILE-017 | approved | Mobile/PWA Context Browse should be treated as an existing entry point into the same workspace browser contract. It may keep its single-pane route (`#/context/{id}/files`) and old `CamApi.listFiles/readFile` calls during migration, but shared API response shape and safety semantics must stay compatible with Desktop Browse. Future mobile Agent Detail Browse should use the agent-scoped workspace endpoints. |
| CAM-DESK-FILE-018 | approved | Desktop Browse v1 polish. Source-file previews must apply lightweight local syntax highlighting (no CDN, no external dependency). At minimum the renderer covers: Python (`.py`), JavaScript (`.js/.mjs/.cjs`), TypeScript (`.ts`, `.tsx`), JSX (`.jsx`), C (`.c/.h`), C++ (`.cpp/.cc/.cxx/.hpp/.hxx`), Java (`.java`), shell (`.sh/.bash/.zsh`), JSON (`.json`), CSS (`.css`), HTML/XML (`.html/.htm/.xml/.svg`), Markdown (`.md/.markdown`), Verilog/SystemVerilog (`.v/.sv/.svh`), YAML (`.yaml/.yml`), and TOML (`.toml`). Highlighting must be bounded — files larger than ~2 MiB or single lines longer than ~800 chars fall back to plain escaped text. All raw content is HTML-escaped at the token boundary; the rendered preview stays selectable and copyable in a `<pre><code>` container. Markdown files additionally render with a Raw / Preview segmented toggle in the preview header. Raw shows the same escaped/selectable `<pre>`; Preview renders a small local markdown subset (ATX headings, hr, fenced code with re-highlighting, inline code, bold/italic/strike, lists, ordered lists, blockquote, links restricted to `http(s):` / `mailto:` / relative, paragraphs). Raw HTML inside the markdown source is escaped, never passed through, and no script tags or `on*` attributes are emitted. The toggle reuses cached content and never triggers a refetch. The output-history affordances (More+, Loading, No more, Jump-to-bottom) must remain hidden while `outputMode === 'browse'`; Browse is workspace files, not tmux output. |

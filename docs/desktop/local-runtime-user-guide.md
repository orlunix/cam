...
| **Relay** | The hub/controller lives behind NAT/firewall and you have a relay endpoint that fronts it. | Nothing. You enter `Relay URL` + `Relay token` only. The CAM API token is now profile-managed on the source side and never asked for in the app. The relay injects the right bearer when forwarding `/api/*`. (Still active.) |
...
| Relay tab connects (websocket up) but agents stay empty / 401 on `/api/...`. | The source-side profile has no `cam_api_token` configured, so the relay can't inject the bearer when forwarding `/api/*`. | Start the source with `camui start --api-token TOKEN` (or `CAMUI_API_TOKEN=TOKEN`) or `camui start --profile NAME` (which generates and persists a stable `cam_api_token` in `~/.cam/camui/relay/<NAME>/profile.json` on first use). Restart the source; the relay will pick up the token. The Desktop/mobile Relay form still only needs Relay URL + Relay token. |
| Relay setup form used to ask for a third "CAM API token" field. | You're on an older Desktop build. | As of 2026-06-12 the Relay setup is exactly two fields: Relay URL + Relay token. If you still see the third field, update Desktop/mobile to the current build. Any saved Relay-mode `cam_token` bearer is ignored/cleared on connect; the Relay token is still used. |

...
| **Relay** | The hub/controller lives behind NAT/firewall and you have a relay endpoint that fronts it. | Nothing. In the active product you enter `Relay URL` + `Relay token`; the source profile owns the CAM API token and the relay injects it. |
...
| Relay tab connects but agents stay empty / 401 on `/api/...`. | Source-side profile token injection is not configured, or the source is offline. | Start the source with `camui start --profile <name> --relay-url ws://<relay> --relay-token <token>` and verify the relay reports `server_connected:true`. The active Desktop Relay tab should only need Relay URL + Relay token. |

            ...
            if os.path.isfile(jsonl):
                try:
                    with open(jsonl, "rb") as f:
                        _add("claude/session.jsonl", f.read())
                except OSError as e:
...
examples:
  ...
  camc version                        Show version info""")
    ...
    ar = sub.add_parser(
        "archive",
        help="Bundle one agent's history (logs + Claude transcript) into a .tar.gz",
    )
    ar.add_argument("id", help="Agent ID")
    ar.add_argument("--output", "-o", default=None,
                    help="Output directory (default: ~/.cam/archives/)")
    ar.add_argument("--session-id", dest="session_id", default=None,

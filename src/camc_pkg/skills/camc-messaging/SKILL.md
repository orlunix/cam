...
is the source of truth; `camc msg read` replays threads from there.
...
| Task | Command |
|---|---|
| Ask another agent (block + reply) | `camc msg send <to> -t "..."` |
| Ask async (return now) | `camc msg send <to> -t "..." --no-wait` |
| Ask async + require reply commit | `camc msg send <to> -t "..." --no-wait --expect-reply` |
| Reply on same thread | `camc msg reply <msg_id> -t "..."` |
| Read inbox | `camc msg read` |
| Read next unread | `camc msg read --next [--mark]` |
| Replay full thread | `camc msg read <msg_id>` |
| Wait for first reply (legacy) | `camc msg wait <msg_id>` |
| Raw ledger entries | `camc msg show <msg_id>` / `camc msg list` |

## 1. Sending a message

`camc msg send <to> --text "..."` is a Bash-tool primitive: by default it
blocks until the target's reply is stable, then prints reply on stdout.
Use as any shell command — `reply=$(camc msg send peer -t "review src/foo.py")`.
For fan-out or fire-and-later workflows, add `--no-wait`; it returns a
message id immediately, and the caller later runs `camc msg read <msg_id>`.

```bash
camc msg send <to> -t "..."             # block; default 600s (10 min) timeout
camc msg send <to> -t "..." --timeout 300       # 5 minutes (seconds, integer only)
camc msg send <to> -t "..." --no-wait   # return MSG_ID + STATUS=sent immediately
camc msg send <to> -t "..." --no-wait --expect-reply
                                        # async + receiver MUST commit reply
```

- Targets: any name in `camc list`
- `--no-wait` means "written to the target tmux pane", not "read" or "understood"
- `--expect-reply` appends `[Reply via: camc msg reply <msg_id> -t "..."]`
...
camc msg read --next                    # print body of oldest unread (msg_id+seq+from+text)
camc msg read --next --mark             # ...and mark it read
camc msg read --all                     # list everything (incl. already-read)
camc msg read --for <agent-id-or-name>  # explicit mailbox (outside-tmux callers)
camc msg read --json                    # stable JSON for tests/automation
camc msg read <msg_id>                  # replay full thread, ordered by seq
camc msg read <msg_id> --mark           # ...and mark unread deliveries in thread
```
...
`camc msg read` will still see it.
...
```bash
camc msg reply <msg_id> -t "..."        # append next seq to the same thread
```

`camc msg reply` does NOT mint a new msg_id — it appends
...
**Prefer the explicit reply path:** `camc msg reply <msg_id> -t "..."`
...
```bash
camc msg wait <msg_id>                  # existing reply or poll for first reply
camc msg show <msg_id>                  # raw ledger entries for one message
camc msg list [--for <to>] [-n 50]      # recent messages summary
```

`wait` is a compatibility helper for the FIRST reply only. For
multi-turn threads, use `camc msg read <msg_id>`.
...
**If the message ends with `[Reply via: camc msg reply <msg_id> -t "..."]`,**
...
**Prefer `camc msg reply <msg_id> -t "..."` even for default sends**
(no `--expect-reply`). It records your reply on the same thread and
is what the sender's `camc msg read <msg_id>` replays.
...
```bash
reply=$(camc msg send reviewer -t "review src/foo.py for bugs")
echo "$reply"
```
...
```bash
id1=$(camc msg send worker1 -t "shard 1" --no-wait | grep MSG_ID | cut -d= -f2)
id2=$(camc msg send worker2 -t "shard 2" --no-wait | grep MSG_ID | cut -d= -f2)
# ... later:
camc msg read $id1
camc msg read $id2
```
...
```bash
camc msg send peer -t "review this" --no-wait --expect-reply
# Receiver MUST run: camc msg reply <msg_id> -t "<answer>"
# Sender polls later: camc msg read <msg_id>
```

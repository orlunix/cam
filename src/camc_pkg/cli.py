...
def cmd_run(args):
    ...
    agent_rec = {
        ...
        "started_at": started_at, "completed_at": None, "exit_reason": None,
        "retry_count": 0, "cost_estimate": None, "files_changed": [],
    }
    if tool == "codex":
        agent_rec["session_binding"] = "pending"
    if not known_tool:
    ...
    if tool == "codex":
        _spawn_codex_session_binder(agent_id, session, workdir, started_at)
...
def cmd_archive(args):
    ...
    Layout of the archive (schema v1):
        ...
        codex/session.jsonl   — validated Codex rollout transcript
    ...
    elif tool == "claude" and (not sid or sid == synthetic_form):
    ...
    # Tool transcript — the main prize
    jsonl_bytes = None
    jsonl_source_path = None
    if tool == "claude" and sid and workdir:
        claude_dir = None
    ...
    elif tool == "codex" and sid and workdir:
        codex_path = _sf(a, "session_path")
        validated = _codex_rollout_metadata(
            codex_path, workdir, expected_id=sid,
        ) if codex_path else None
        if not validated:
            print_warning("Codex rollout binding is invalid; skipping Codex transcript")
        else:
            try:
                with open(codex_path, "rb") as f:
                    jsonl_bytes = f.read()
                jsonl_source_path = codex_path
                _add("codex/session.jsonl", jsonl_bytes)
            except OSError as e:
                log.warning("read %s: %s", codex_path, e)
    ...
    if jsonl_bytes and tool == "claude":
        try:
...
_CODEX_ROLLOUT_RE = re.compile(
    r"^rollout-.+-(%s)\.jsonl$" % _UUID_RE_STR,
)
_CODEX_BIND_ATTEMPTS = 120  # 30 seconds at the fixed 250 ms poll interval


def _proc_env(pid):
    """Return a process environment mapping, or an empty mapping on failure."""
    if not pid:
        return {}
    try:
    ...
    for entry in entries:
        try:
            key, value = entry.decode("utf-8", errors="surrogateescape").split("=", 1)
        except ValueError:
            continue
        env[key] = value
    return env
...
def _parse_iso_timestamp(value):
    """Parse an ISO-8601 timestamp to UTC, returning None for malformed input."""
...
def _codex_rollout_metadata(path, expected_workdir, expected_id=None,
                            started_at=None):
    """Return a validated Codex rollout UUID, or None without guessing.

    The caller is responsible for proving process ownership of ``path``.  This
    helper only validates the filename and first ``session_meta`` record.
    """
    match = _CODEX_ROLLOUT_RE.match(os.path.basename(path))
    if not match:
        return None
    file_id = match.group(1)
    if expected_id and file_id != expected_id:
        return None
    try:
    ...
    if record.get("type") != "session_meta" or not isinstance(payload, dict):
        return None
    if payload.get("session_id") != file_id:
        return None
    cwd = payload.get("cwd")
    if not isinstance(cwd, str) or os.path.realpath(cwd) != os.path.realpath(expected_workdir):
        return None
    if started_at:
        created = _parse_iso_timestamp(payload.get("timestamp") or record.get("timestamp"))
...
def _validated_codex_rollout(pid, workdir, started_at):
    """Return ``(session_id, path)`` for one rollout opened by this PID.

    This deliberately does not search Codex's session directory: an open file
    descriptor is the process-level evidence that prevents same-workdir or
    concurrent-launch sessions from being confused with this agent.
    """
    if not pid or not workdir:
        return None
    env = _proc_env(pid)
    home = env.get("CODEX_HOME") or os.path.join(os.path.expanduser("~"), ".codex")
    sessions_root = os.path.realpath(os.path.join(home, "sessions"))
    ...
    for fd in fds:
        ...
        try:
            ...
            continue
        session_id = _codex_rollout_metadata(path, workdir, started_at=started_at)
        if session_id:
            candidates.add((session_id, path))
    if len(candidates) != 1:
        return None
...
def _proc_children(pid):
    """Return direct child PIDs using the host's process table."""
    try:
        proc = subprocess.run(
            ["pgrep", "-P", str(pid)], stdout=subprocess.PIPE,
    ...
    if proc.returncode not in (0, 1):
        return []
    out = []
    for value in proc.stdout.decode("ascii", errors="ignore").split():
        try:
...
def _is_codex_process(pid):
    """Return whether the executable/wrapper argv identifies Codex."""
    try:
        ...
        return False
    return any("codex" in os.path.basename(part).lower() for part in argv[:3])


def _find_codex_pids_for_session(session):
    """Return Codex PIDs descended from this exact tmux pane, if any."""
    if not session:
        return []
    socket = _find_tmux_socket(session)
    if not socket:
        return []
    try:
        from camc_pkg.transport import _tmux_bin_for_session
        tmux = _tmux_bin_for_session(session) or "tmux"
        rc, out = _run(
            [tmux, "-u", "-S", socket, "list-panes", "-t", session,
             "-F", "#{pane_pid}"],
            timeout=5,
        )
    except Exception:
    ...
    if rc != 0:
        return []
    pending = []
    for value in (out or "").split():
        try:
            pending.append((int(value), 0))
        except ValueError:
            pass
    seen, codex_pids = set(), []
    while pending and len(seen) < 64:
        ...
        if depth < 8:
            pending.extend((child, depth + 1) for child in _proc_children(pid))
    return codex_pids


def _bind_codex_session_once(agent_id, tmux_session, workdir, started_at,
                              store=None):
    """Bind one Codex agent only when PID-level evidence is unique.

    The second record read makes a delayed binder harmless if the agent was
    removed or replaced while its process tree was being inspected.
    """
    store = store or AgentStore()
    agent = store.get(agent_id)
    if not agent or _tf(agent.get("task", {}), "tool") != "codex":
        return False
    if (_sf(agent, "tmux_session") != tmux_session
            or _sf(agent, "context_path") != workdir
            or _sf(agent, "started_at") != started_at):
        return False
    candidates = set()
    for pid in _find_codex_pids_for_session(tmux_session):
        found = _validated_codex_rollout(pid, workdir, started_at)
        if found:
            candidates.add(found)
    if len(candidates) != 1:
        return False
    session_id, session_path = next(iter(candidates))
    current = store.get(agent_id)
    if (not current or _tf(current.get("task", {}), "tool") != "codex"
            or _sf(current, "tmux_session") != tmux_session
            or _sf(current, "context_path") != workdir
            or _sf(current, "started_at") != started_at):
        return False
    store.update(agent_id, session_id=session_id, session_path=session_path,
                 session_binding="bound")
    return True


def _run_codex_session_binder(agent_id, tmux_session, workdir, started_at):
    """Bounded detached worker for a newly created Codex agent."""
    store = AgentStore()
    for attempt in range(_CODEX_BIND_ATTEMPTS):
        if _bind_codex_session_once(agent_id, tmux_session, workdir,
                                    started_at, store=store):
            return 0
        if attempt < _CODEX_BIND_ATTEMPTS - 1:
            time.sleep(0.25)
    current = store.get(agent_id)
    if (current and _tf(current.get("task", {}), "tool") == "codex"
            and _sf(current, "tmux_session") == tmux_session
            and _sf(current, "context_path") == workdir
            and _sf(current, "started_at") == started_at
            and not current.get("session_id")):
        store.update(agent_id, session_binding="unavailable")
    return 1

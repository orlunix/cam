...
| CAM-DESK-TODOS-011 | approved | The mode has exactly five tabs: **Inbox**, **Tasks**, **Notes**, **Projects**, **Archive**. Each tab is a structural slice of the same underlying worklog rows — Inbox = newly captured items pending triage, Tasks = checklist-bearing rows, Notes = free-form Markdown rows, Projects = grouping containers, Archive = soft-deleted rows. A row's tab membership is derived from its kind + status; the user never explicitly moves a row between Inbox and Tasks. |
...
| CAM-DESK-TODOS-014 | approved | Two primary actions in the header: **Add Task** and **Add Note**. Both open inline composer rows at the top of the list (no modal). Add Task requires a title; Add Note requires body text. Both produce Markdown-backed rows immediately visible in the appropriate tab without leaving the page. |
| CAM-DESK-TODOS-015 | approved | The detail panel for a selected row exposes four placeholder sub-tabs: **Preview**, **Raw**, **Checklist**, **History**. Preview renders the row's Markdown (V0 may emit literal text in a `<pre>` block — full rendering is later work). Raw shows the editable Markdown source. Checklist shows the row's task-list lines as toggleable checkboxes. History shows a stub timeline placeholder ("No history yet" in V0). All four tabs must render without crashing on rows whose underlying data is missing those fields. |
| CAM-DESK-TODOS-016 | approved | The V0 data model is a single TypeScript record shape `{ id, kind, title, body, status, tags, project, created_at, updated_at }` persisted to `localStorage` under a stable key so users can dogfood the UI without a backend. The model deliberately matches a future Markdown-on-disk worklog schema: each row's `body` is treated as Markdown; checklist items are parsed from `- [ ]` / `- [x]` lines. A future slice replaces the localStorage store with a Hub-mediated `todocli` API; the renderer must not encode that backend's URL shape in the V0 store interface. |
...
| CAM-DESK-TERM-004 | approved | Lifecycle + fast switch. Terminal mode keeps a small per-renderer cache of recent per-agent xterm/PTY sessions (default cap: 6). Selecting another agent in Terminal mode shows that agent's cached xterm pane and existing SSH PTY channel when available; it must not close/reopen the previous channel or display a reconnect banner for cached agents. Opening an already-attached agent returns the existing session id from Electron main. Eviction, app shutdown, Direct→Relay/no-preload capability loss, or explicit close sends `term:close` → `stream.end()` / `stream.destroy()` on the channel. Detaching the local view does **not** kill the tmux session and does **not** stop the underlying agent — `camc attach` is a tmux-level attach. On `before-quit`, every cached channel is disposed before the embedded Hub teardown so pooled clients can flush END/CLOSE frames cleanly. |
| CAM-DESK-TERM-005 | approved | Renderer focus. Terminal mode is the default Agent Detail output mode when Direct terminal capability is available; Relay/browser/no-preload environments fall back to Rich so the UI never lands on a disabled terminal. While Terminal mode is active, the composer surface is hidden completely — no quick-key tab, attachment button, input textarea, or Send button — so xterm owns keyboard input. The Terminal pane uses the full available Agent-detail output width, not the Plain/Rich reading rail. The xterm host must not add layout padding that can make the fit addon over-count rows; the renderer refits after Terminal mode becomes visible and after resize/layout events so the last terminal row remains fully visible even when the Desktop window is not maximized. Plain↔Rich switching restores the composer. The Terminal button has an inline disabled tooltip ("Terminal mode is available only with Direct connection") when the active connection mode is Relay-only. Per-pane state machines (output history, autoScroll, viaPoll suppression) remain Plain/Rich only — Terminal mode bypasses them entirely because the channel is push-based. |
| CAM-DESK-TERM-006 | approved | Terminal mode must provide a floating attachment affordance inside the terminal output wrapper because the normal composer is hidden. Attachment upload reuses the existing `CamApi.uploadFile()` endpoint and inserts the returned remote workspace path as a lightweight marker ` [attached: <remote-path>] ` through `CamBridge.term.input({sessionId,data})`; it must not call `sendInput`, mutate the hidden composer textarea, insert base64, insert local filesystem paths, or auto-submit Enter. Terminal `Ctrl+V` is handled text-first: if the local Electron clipboard has text, paste that text into xterm; otherwise, if the clipboard has a local file/image, upload it locally and insert the same attachment marker. `Ctrl+Shift+V` remains xterm/browser default text paste. The explicit Attach button stays available and follows the same marker format. The feature is Direct/Terminal-session scoped: disabled while the terminal channel is connecting or detached, hidden outside Terminal mode, and status/errors are shown in the floating output controls rather than the hidden composer upload status. |
| CAM-DESK-TERM-007 | approved | Terminal tab sizing guard. Desktop Terminal mode models recent agents like Windows Terminal tabs: at most 6 recent per-agent terminal sessions remain live per renderer, each with an independent xterm buffer and independent SSH PTY attach channel multiplexed over the pooled ssh2 client. Hidden terminal tabs must never auto-fit or send resize events to the remote PTY; only the active visible tab may resize the remote channel. During agent switch, the target pane may be temporarily `visibility:hidden` while still participating in layout so xterm-fit can measure the real width before display; it must not use `display:none` / `hidden` for fit measurement. Any resize below 40 columns or 4 rows is treated as an invalid layout artifact and ignored in both renderer and Electron main. Before opening a fresh terminal attach, Electron main runs a best-effort remote tmux repair against the selected agent: read `camc --json status <agent>`, detach existing tmux clients narrower than 40 columns or shorter than 4 rows, and resize the agent tmux window to the active terminal size before `camc attach` using tmux control-mode `refresh-client -C <cols>,<rows>` so the repair works on tmux 2.7 (the main PDX environment) as well as newer tmux versions. Reusing a cached terminal session keeps switching fast and schedules the same repair asynchronously. LRU eviction closes the oldest cached terminal channel after the cap is exceeded; closing a cached attach detaches only the local `camc attach` client and never kills the underlying tmux agent session. Regression guard: terminal tab caching must not trigger agent-list refresh, Plain/Rich polling, remote context sync, or tmux-size changes from hidden tabs. |
...
| CAM-DESK-WORKFLOW-011 | approved | The primary visual surface is a full-width foldable card list aligned with the Nodes page style. Each workflow node is one editable card: collapsed cards show node id/name, run type or skill (`run.skill`, command runner, evaluator-related node when present), dependency/edge summary, checklist count, expected-output field count, verify mode, retry count, and runtime state when available; expanded cards expose inline editors for the important fields. Edges are derived from `needs` / dependency fields and transition-like fields when present. A future graph canvas may be added, but V0 prioritizes stable card editing over decorative connectors. |
